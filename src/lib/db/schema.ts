...
export const chatShares = pgTable(
  ...
  (table) => ({
    ...
    sharedByUserIdIdx: index("chat_shares_shared_by_user_id_idx").on(
...
export const chats = pgTable(
  ...
  (table) => ({
    ...
    lastMessageAtIdx: index("chats_last_message_at_idx").on(
      table.lastMessageAt
    ),
    titleIdx: index("chats_title_idx").on(table.title), // For search
    // Composite index for common query patterns
    userUpdatedIdx: index("chats_user_updated_idx").on(
      table.userId,
      table.updatedAt
    ),
    userFiltersIdx: index("chats_user_filters_idx").on(
      table.userId,
      table.isArchived,
      table.isPinned,
      table.updatedAt
...
export const files = pgTable(
  ...
  (table) => ({
    ...
    chatCreatedIdx: index("files_chat_created_idx").on(
      table.chatId,
      table.createdAt
    ),
  })
...
export const userProviders = pgTable(
  ...
  (table) => ({
    ...
    userProviderIdx: index("user_providers_user_provider_idx").on(
      table.userId,
      table.provider
    ),
  })
