...
async function getHandler(request: NextRequest, user: User) {
  ...
  const debugData = providers.map((provider, index) => {
    ...
    if (timeSinceValidation) {
      console.log(`  Time Since Validation: ${Math.round(timeSinceValidation / (1000 * 60))} minutes`);
    }
  ...
  const issues = [];
  const pendingProviders = providers.filter(p => p.validationStatus === 'pending');
  const invalidProviders = providers.filter(p => p.validationStatus === 'invalid');
  const providersWithKeysButNeverValidated = providers.filter(p => 
    p.encryptedApiKey && !p.lastValidated
  );
  const oldValidations = providers.filter(p => 
    p.lastValidated && (Date.now() - new Date(p.lastValidated).getTime()) > (24 * 60 * 60 * 1000)
  );
  
  if (pendingProviders.length > 0) {
    issues.push(`${pendingProviders.length} provider(s) stuck in 'pending' status`);
  }
  if (invalidProviders.length > 0) {
    issues.push(`${invalidProviders.length} provider(s) marked as 'invalid'`);
  }
  if (providersWithKeysButNeverValidated.length > 0) {
    issues.push(`${providersWithKeysButNeverValidated.length} provider(s) have API keys but were never validated`);
  }
  if (oldValidations.length > 0) {
    issues.push(`${oldValidations.length} provider(s) haven't been validated in over 24 hours`);
  }
