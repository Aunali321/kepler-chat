...
async function postHandler(request: NextRequest, user: User) {
  ...
  for (const provider of providers) {
    ...
    try {
      ...
      const validation = await validateApiKey(provider.provider, decryptedKey);
      ...
      if (provider.validationStatus !== newStatus) {
        console.log(`  Updating status: ${provider.validationStatus} -> ${newStatus}`);
        
        await updateUserProvider(user.id, provider.provider, {
        ...
        results.push({
          ...
          newStatus: newStatus,
          reason: validation.isValid ? 'API key is valid' : 'API key validation failed',
        });
      } else {
